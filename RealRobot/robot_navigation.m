...
%botSim.setBotPos([43 25]);
botSim.drawBot(5,'yellow');
...
ghostBot = returnedBot; %BotSim(map, [0,0,0]);
...
while Robot_location(1) > end_point(1) + 0.002 || Robot_location(1) < end_point(1) - 0.002 || Robot_location(2) > end_point(2) + 0.002 || Robot_location(2) < end_point(2) - 0.002
    ...
    bot.move(r, distance/100);
    ghostBot.move(distance);
    Robot_location = ghostBot.getBotPos();
    
    %botScanRaw = survey(r, 50, 60)*(60/26)
