function inflated_boundaries = boundary_inflation(external_boundaries, robot_size)
% Function creates inflated internal boundaries which are an inflated 
% version of the given exteranl boundaries the robot cannot cross.
% It is based on the size of the robot, robot_size. 

%% Initialize empty arrays to be used to create the inflated boundaries 
...
%% Extract adjacent external boundary vertices
for current_wall_index = 1:1:size(external_boundaries,1)
    ...
    % Calculate midpoint of current external boundary wall 
    midpoint_x(current_wall_index) = (x_1 + x_2)/2;
    midpoint_y(current_wall_index) = (y_1 + y_2)/2;
    
    % Calculate orientation of current external boundary wall
    orientation(current_wall_index) = atan( (y_2 - y_1)/(x_2 - x_1) );
    
    % Copy midpoint and shift by amount equal to robot_size, perpendicular to
    ...
    % IN = 1 means particles are inside the wall, IN = 0 means particles are outside the walls
